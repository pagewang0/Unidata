    ...
    async get(options) {
        ...
        const result = {
            ...
            list: response.list.map((item) => {
                ...
                const body = item.attachments?.find((attachment) => attachment.name === 'description');
                if (body) {
                    item.body = body;
                    item.attachments = item.attachments.filter((attachment) => attachment.name !== 'description');
