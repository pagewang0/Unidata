...
class GitcoinContribution extends base_1.default {
    ...
    async get(options) {
        ...
        let merged = (0, lodash_es_1.keyBy)([list[0]], (item) => item.metadata?.proof);
        for (let i = 1; i < list.length; i++) {
            merged = (0, lodash_es_1.mergeWith)(
                merged,
                (0, lodash_es_1.keyBy)([list[i]], (item) => item.metadata?.proof),
                (a, b) => {
                    if (Array.isArray(a)) {
                        return (0, lodash_es_1.uniqWith)(a.concat(b), lodash_es_1.isEqual);
                    }
                },
            );
        }
