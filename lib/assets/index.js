...
class Assets {
    ...
    async get(options) {
        ...
        if (options.providers.length > 1) {
            ...
            let merged = (0, lodash_es_1.keyBy)(list[0].list, (item) => item.metadata?.proof);
            for (let i = 1; i < list.length; i++) {
                merged = (0, lodash_es_1.mergeWith)(
                    merged,
                    (0, lodash_es_1.keyBy)(list[i].list, (item) => item.metadata?.proof),
                    (a, b) => {
                        if (Array.isArray(a)) {
                            return (0, lodash_es_1.uniqWith)(b.concat(a), lodash_es_1.isEqual);
                        }
                    },
                );
            }
            const assets = (0, lodash_es_1.values)(merged);
